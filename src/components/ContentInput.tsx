 ...
 *
 * Handles document upload and text input for the application.
 * Supports file drag-and-drop, file selection, and direct text input with LaTeX support.
 */
...
/** LaTeX placeholder text for the textarea */
const LATEX_PLACEHOLDER = `Paste your content here... LaTeX equations are fully supported:
...
const ContentInput: React.FC<ContentInputProps> = ({ documents, setDocuments }) => {
  ...
  /**
   * Processes uploaded files and adds them as documents
   */
  ...
  /**
   * Adds text input as a new document
   */
  const addTextDocument = (): void => {
    ...
    <div className="p-10 dark:bg-dark-background dark:text-dark-text space-y-8">
      ...
      <div className="mb-10">
        ...
        <p className="text-gray-600 dark:text-dark-text-secondary text-lg">
          Upload multiple documents or paste content directly. Supports LaTeX for STEM content.
        </p>
      </div>
        ...
        <div>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {documents.map((doc) => (
              <div
                key={doc.id}
                className="border border-gray-200 rounded-lg p-6 bg-gray-50 dark:border-dark-input-border dark:bg-dark-surface"
              >
